...
export default class Trie extends Node {
  ...
  findSuggests(currNode, string, suggests = []) {
    ...
    childrenletters.forEach( childletter => {
      ...
      if (currNode.children[childletter].wordEnd) {
        suggests.push({word: newString, count: currNode.children[childletter].frequency});
      }

      this.findSuggests(currNode.children[childletter], newString, suggests);
    });
